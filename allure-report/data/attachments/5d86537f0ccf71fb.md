...
> 21 |         await expect(await pages.homePage.welcomeMessage()).toBeVisible();
     |                                                             ^ Error: toBeVisible can be only used with Locator object
  22 |         await expect(await pages.homePage.welcomeMessage()).toContain(username);
  23 |         await pages.homePage.selectProduct(productName);
  24 |     })
  25 |     
  26 |     await allure.step('Landing in Cart Page', async() => {
  27 |         await pages.cartPage.addToCart();
  28 |     })
  29 | });
```